...
columnText = `<div class="pair">
	...
	<input name="Input Type" type="checkbox" id="not">
	<label>Not</label>
...
function AddRow(addButton) {
	newRowClone = newRow.cloneNode(true)
	insertAfter(newRowClone, addButton)
	AddColumn(newRowClone.childNodes[3])
}

function AddColumn(addButton) {
	newColumnClone = newColumn.cloneNode(true)
	insertAfter(newColumnClone, addButton)
